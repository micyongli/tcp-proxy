...
func crc(buf []byte, index int, size int, initValue uint16) uint16 {
	crcInitValue := initValue
	for i := 0; i < size; i++ {
		v := buf[i+index]
		tmp := crcInitValue ^ uint16(v)
		tmp &= 0xff
		crcInitValue >>= 8
		crcInitValue ^= table[tmp]
	}
	return crcInitValue
...
// Crc16 校验计算
func Crc16(buf []byte, index int, size int) uint16 {
	return crc(buf, index, size, 0xffff)
}

func ReadUInt16BE(buf []byte, index int) int {
	return (int(buf[index]) << 8) + int(buf[index+1])
}
...
func WriteUInt32BE(v uint32, buf []byte, index int) int {
	buf[index] = byte((0xff000000 & v) >> 24)
	buf[index+1] = byte((0xff0000 & v) >> 16)
	buf[index+2] = byte((0xff00 & v) >> 8)
	buf[index+3] = byte(0xff & v)
	return 4
}

func Int2HexStr(v int) string {
	b := make([]byte, 4)
	WriteUInt32BE(uint32(v), b, 0)
	return hex.EncodeToString(b)
}

func ReadUInt64BE(buf []byte) uint64 {
	var k uint64 = 0
	m := 8
	for i, v := range buf {
		if i < m {
...
func WriteUInt16BE(v int, buf []byte, index int) int {
	buf[index+1], buf[index] = byte(v&0xff), byte(v>>8)
	return 2
}

func ReadUInt16LE(buf []byte, index int) int {
	return (int(buf[index+1]) << 8) + int(buf[index])
}

func WriteUInt16LE(v int, buf []byte, index int) int {
	buf[index], buf[index+1] = byte(v&0xff), byte(v>>8)
	return 2
}

func RemoveStrSep(s string, sep string) string {
	return strings.ReplaceAll(s, sep, "")
}

func RemoveStrSpace(s string) string {
	return RemoveStrSep(s, " ")
}

func Cp(s []byte, si int, t []byte, ti int, size int) int {
	for i := 0; i < size; i++ {
		t[i+ti] = s[si+i]
	}
	return size
...
func ToStringOnlyPrintChar(b []byte) string {
	var s []string
	for _, v := range b {
